...
function xlsx(file) {
	...
	function getAttr(s, n) {
		...
		// replaced by regex because of false positive on 't="' vs 'left="'
		const exp = new RegExp(`^${n}\=\"(\\w*)\"|\\s${n}\=\"([^\"]*)\"`)
		const match = s.match(exp)
		if (!match) return ""
		return match[1] === undefined ? ( match[2] === undefined ? "" : match[2] ) : match[1]
	}
   ...
   if (typeof file === 'string') { // Load
		...
		result = {
			zip: zip,
			blob: function() { return this.zip.generate({ type: 'blob' })},
			data: function() { return this.zip.generate({ type: 'uint8array' }) },
			base64: function() { return this.zip.generate({ compression: 'DEFLATE' }) },
